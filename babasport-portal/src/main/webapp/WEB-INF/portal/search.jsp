...
<script type="text/javascript">
	...
	function fqBrand(bId){
		if (price!=null && price !='') {//说明已经选了价格
			window.location = "/Search?price="+price+"&brandId="+bId+"&keyword="+keyword;
		}else{//没有选价格
			window.location = "/Search?brandId="+bId+"&keyword="+keyword;
		}
	}
	
	//根据价格筛选
	function fqPrice(pri){
		if (brandId!=null && brandId !='') {//已经选择品牌
			window.location = "/Search?price="+pri+"&brandId="+brandId+"&keyword="+keyword;
		}else{//没有选择品牌
			window.location = "/Search?price="+pri+"&keyword="+keyword;
		}
	}
</script>
</head>
		...
		<li data-sku="1711416562" class="gl-item">
			<div class="gl-i-wrap">
				...
				<div class="p-name p-name-type-2">
					<a onclick="window.open('http://localhost:8084/html/product/${product.id}.html')" target="_blank" title="满129立减10,199减20优惠券,支持货到付款" href="javascript:;" onclick="">
						<em>${product.name }</em>
					</a>
				</div>
				<div class="p-commit">
